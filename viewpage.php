...
if($row_getpage['isshow']=="-1"){screenMessage("This post is hidden","The post is not open, it is hidden by the moderator.");}
if($row_getpage['isshow']=="0"){screenMessage("This post is hidden","The post is not open, it is hidden by the author.");}
    $blogInfo = dbRow("SELECT * FROM zb_user WHERE id = ".$row_getpage['ownerid']);

	$owner=$row_getpage['ownerid'];
	if(!$row_getpage){screenMessage("Error","This article had been deleted or not exists.");}
	...
	if ($_POST["reply_blog"] == "1") {
		...
		if ($result["success"] != true && $owner != $gId) {$error = 1;}
		if($error != 1){
			...
			$insertSQL = sprintf("INSERT INTO zb_comment (name, email, content,is_whisper, `time`, pageid, verified, ip) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
            ($blogInfo["login_to_comment"]==1 && $isLog)?GetSQLValueString($gUsername, "text"):GetSQLValueString($_POST['name'], "text"),
			GetSQLValueString($_POST['email'], "text"),
			GetSQLValueString($_POST['content'], "text"),
			isset($_POST['is_whisper'])?"1":"0",
			GetSQLValueString(gmdate("Y-m-d H:i:s", time()+28800), "date"),
			GetSQLValueString($_GET['tid'], "int"),
			GetSQLValueString($verified, "int"), 
	...
	if($blogInfo['comment_system1'] == 1){
		//get comments
		$recentComments = dbAr("SELECT b.id, SUBSTR(a.content,1,30) as conbar FROM zb_comment a, zb_contentpages b where b.isshow = 1 AND is_whisper = 0 AND a.pageid = b.id AND b.ownerid = {$blogInfo['id']} ORDER BY a.time DESC LIMIT 15",60*2);
		...
		if($z){
			foreach($z as $v){
			$hotReply[]="<a href='http://realblog.zkiz.com/{$blogInfo['username']}/{$v['id']}'>".htmlspecialchars($v['title'])."...</a>";
			}
		}
