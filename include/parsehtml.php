...
if ((!in_array($tagName, $this->emptyTags)) && $isStartTag &&$this->blockElements[$tagName] && (!$tagEnd  = strpos($this->html,'</'.$tagName.'>'))) {
$this->invalidTag();
return false;
}
...
$this->node = preg_replace('# */? *>$#', ' />', $this->node);//royze 也是有可能是错误的吧。
}
$this->nodeType = 'tag';
...
/**
* handle invalid tags 处理无效的 tag 函数
*
* @param void
* @return void
*/
function invalidTag() {
//查找 无效标签的结尾 >
$tagend = strpos($this->html,'>') + 1;
//再查找 下一个标签的开始
$nexttag = strpos($this->html,'<',1);
//哪一个小就用哪一个作为替换的结尾。
$validstart = $tagend>$nexttag?$nexttag:$tagend;
if ($validstart) {
$this->html = substr($this->html, $validstart);
} else { //如果是没有结尾的话，就将这以后的全部都替换为空了
$this->html = '';
}
